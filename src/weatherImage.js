...
export const getIconPath = (code, timeOfDay) => {
  timeOfDay = timeOfDay == 0 ? "night" : "day";
  const fileName = weather_codes[code]?.icons?.[timeOfDay];
  return fileName ? `/assets/${fileName}` : null;
};

export const getClimateDetails = (code) => {
